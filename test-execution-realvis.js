...
async function testFullExecution() {
    ...
    console.log("Submitting job (RealVis)...");
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (history[prompt_id]) {
            ...
            if (history[prompt_id].status.status === 'error' || history[prompt_id].status.completed === false) {
                console.log("❌ ERROR DETAILS:", JSON.stringify(history[prompt_id].status, null, 2));
            } else {
                console.log("✅ DONE!");
            }
            return;
