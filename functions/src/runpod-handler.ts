...
interface PodInfo {
  id: string;
  proxyUrl: string; // RunPod proxy URL (e.g., wss://podId-8188.proxy.runpod.net/ws)
  status: string;
}
...
export async function getPodStatus(podId: string): Promise<PodInfo> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (status === "RUNNING" && pod?.runtime) {
      // RunPod uses a proxy service for external connections
      // Format: wss://{podId}-{port}.proxy.runpod.net/ws
      const proxyUrl = `wss://${podId}-8188.proxy.runpod.net/ws`;
