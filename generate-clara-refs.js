...
// Configuration
const SOURCE_IMAGE = '1769946490115-image_(1).jpg';
...
const CLARA_TRAITS = "(extreme hourglass figure:1.2), (huge breasts:1.7), (curvy:1.5), (very wide hips:1.3), (slim waist:1.1), (thick thighs:1.4), (soft body:1.3), (fleshy:1.1), (straight hair:1.2), (round soft face:1.2), long honey-brown hair, glasses, soft pretty facial features";
...
function constructWorkflow(posePrompt, uploadedImageName, seed) {
...
async function generateReferences() {
    ...
    console.log(`Target: 20 Images | Output: ./output/clara_refs/`);

    const localPath = path.join(__dirname, 'public/uploads', SOURCE_IMAGE);
    const uploadedImageName = await uploadImage(localPath, `MASTER_REF_CLARA.jpg`);

    if (!uploadedImageName) {
        console.error("❌ Critical: Could not upload source image.");
        return;
    }
    ...
    for (const p of PROMPTS) {
        ...
        const workflow = constructWorkflow(p.text, uploadedImageName, seed);
